...
export default {
  props: {
    ...
    multipleApi: {
      type: String,
      default: 'admin/goods/@ids/@sellerId/sn/details'
    },
    // 分类API
    categoryApi: {
      type: String,
  ...
  mounted() {
    request({
      url: 'admin/goods/categories/0/children',
      method: 'get',
      loading: false,
      params:{
        page_no: 1,
    ...
    request({
      ...
      //   this.value = this.options[0].shop_id
      //   this.params.sellerId = this.options[0].shop_id
      // }
    })
  ...
  methods: {
    ...
    defaultDataChanged(newVal) {
      ...
      if (newVal && newVal.length > 0) {
        this.GET_GoodsByGoodsIds(newVal/*,this.value*/).then(response => this.selectedList.push(...response))
      }
    },
    /** 计算是否已被选 */
    isSelected(goods) {
    ...
    handleSearchMultipleGoods() {
      ...
      if (goods_ids != '') {
        this.GET_GoodsByGoodsIds(goods_ids/*,this.value*/).then(response => this.goodsList = response)
      } else {
    ...
    changeClassify(value){
      this.params.category_path = value
    },
    optionChange(value){
      this.params.sellerId = value
      this.goodsList = []
      this.selectedList = []
    }
  }
