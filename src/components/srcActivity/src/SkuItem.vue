    ...
    methods: {
      ...
      fetCategory(){
        if(this.$parent.$attrs.indexH == 1000) {
          this.getGroupbuyCategorySkuList()
        }else{
      ...
      /** 活动---根据分类id获取规格信息*/
      getGroupbuyCategorySkuList() {

        if (this.categoryId != undefined) {
          API_goodsSku.getGroupbuyCategorySkuList(this.categoryId, {}).then(response => {
            this.skuData = response
          })
        }
      },
      ...
      editSkuItem(item, $index) {
        ...
        if(this.$parent.$attrs.indexH == 1000){
          /** 更新用户自定义规格项  */
          API_goodsSku.saveGroupbuyCustomSkuItem(this.categoryId, { spec_name: item.spec_name }).then(response => {
