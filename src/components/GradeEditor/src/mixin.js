  ...
  methods: {
    ...
    GET_ChildrenById(id) {
      ...
      request({
        ...
        if (data[0].hasOwnProperty('cmi_proportion')) {
          // 参数名映射
          data.map(item => {
            item.$id = item[names.id]
            if (item.cmi_proportion == null) {
              item.cmi_proportion = ''
            }
            item.$text = item[names.text] + '           ' + item.cmi_proportion
            return item
          })
        } else {
          // 参数名映射
    ...
    refresh(type, item) {
      ...
      if (type === 'edit' && item) {
        ...
        if (item.hasOwnProperty('cmi_proportion')) {
          item[paramsNames.text] && (_item.$text = item[paramsNames.text] + '    ' + item.cmi_proportion)
        } else {
