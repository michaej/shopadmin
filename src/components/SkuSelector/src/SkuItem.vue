  ...
  export default {
    ...
    mounted() {
      if(this.isActivity == 1){
        this.getActivityCategorySkuList()
      }else{
    ...
    watch: {
      ...
      categoryId: 'fetCategory'
    },
    methods: {

      fetCategory(){
        if(this.$parent.$attrs.indexH == 1000) {
          this.getActivityCategorySkuList()
        }else{
          this.getSkuInfoByCategory()
        }
      ...
      getSkuInfoByCategory() {
        console.log(this.$parent.$attrs.indexH)
        console.log(this.isActivity)
        
        if (this.categoryId && this.categoryId > 0) {
          API_goodsSku.getCategorySkuList(this.categoryId, {seller_id:this.sellerId}).then(response => {
            this.skuData = response
          })
        }
      },
      /** 活动---根据分类id获取规格信息*/
      getActivityCategorySkuList() {
        if (this.categoryId && this.categoryId > 0) {
          API_goodsSku.getActivityCategorySkuList(this.categoryId, {seller_id:this.sellerId}).then(response => {
            this.skuData = response
          })
        }
      },
      ...
      getSkuInfo() {
        ...
        console.log(this.skuInfo,123456)
        if (this.categoryId && this.categoryId > 0) {
          API_goodsSku.getActivityCategorySkuList(this.categoryId, {seller_id:this.sellerId}).then(response => {
          // API_goodsSku.getCategorySkuList(this.categoryId, {seller_id:this.sellerId}).then(response => {
            this.skuData = response
            console.log(response,654321)
      ...
      editSkuItem(item, $index) {
        ...
        if(this.$parent.$attrs.indexH == 1000){
          /** 更新用户自定义规格项  */
          API_goodsSku.saveActivityCustomSkuItem(this.categoryId, { spec_name: item.spec_name,seller_id:this.sellerId }).then(response => {
           ...
           /** 更新用户自定义规格项  */
          API_goodsSku.saveCustomSkuItem(this.categoryId, { spec_name: item.spec_name,seller_id:this.sellerId }).then(response => {
            this.$message.success('添加自定义规格项成功')
            /** 更新下拉列表规格项数据 */
            this.getSkuInfoByCategory()
            /** 更新skuInfo数据 */
      ...
      editSkuIValue(item, val, $index, index) {
        ...
        /** 更新用户自定义规格值 */
        API_goodsSku.saveCustomSkuValue(item.spec_id, { value_name: val.spec_value,seller_id:this.sellerId }).then(response => {
