<template>
  <div v-loading="loading" class="order-detail-container">
    ...
    <el-row v-if="productList" :gutter="0">
      <el-col :span="24">
        <div class="d-header">服务列表</div>
        <el-table :data="productList" :header-cell-style="{textAlign: 'center'}">
          <el-table-column label="服务图片" width="180">
          ...
          <el-table-column label="服务价格" width="150">
            <template slot-scope="scope">￥{{ scope.row.total_price | unitPrice }}</template>
          </el-table-column>
  ...
  export default {
    ...
    methods: {
      GET_OrderDetail() {
        ...
        let { seller_id } = this.$route.params
        let params = {
          seller_id: seller_id
        }
        API_appointment.getBillsItems(params).then(response => {
          this.loading = false
      ...
      GET_OrderLog() {
        ...
        API_appointment.getOrderLog(this.sn).then(response => {
          this.loading_log = false
          this.orderLog = response.log
        }).catch(() => { this.loading_log = false })
      },

      /** 获取订单分销信息 */
      GET_SaleOrg() {
        API_appointment.getSaleOrg(this.sn).then(response => {
      ...
      confirmPay() {
        this.$confirm('确定要确认收款吗？', '提示', { type: 'warning' }).then(() => {
          API_appointment.confirmPay(this.sn, this.orderDetail.order_price).then(response => {
      ...
      cancelOrder() {
        this.$confirm('确定要取消这个订单吗？', '提示', { type: 'warning' }).then(() => {
          API_appointment.cancleOrder(this.sn).then(() => {
          ...
          [
            {
              ...
              key: 'base',
              items: [
                { label: '服务编号', value: o.order_no },
                { label: '店铺名称', value: o.shop_name },
                { label: '下单时间', value: f.unixToDate(o.create_time) },
                { label: '完成时间', value: f.unixToDate(o.end_time) },
                { label: '服务金额', value: '￥' + f.formatPrice(o.business_money) },
                { label: '收益', value: '￥' + f.formatPrice(o.profit_price) },
                { label: '平台服务费', value: '￥' + f.formatPrice(o.service_price) },
                { label: '上级分成', value: o.superior_price },
                { label: '服务状态', value: o.order_status == 0 ? '删除' : (o.order_status == 1 ? '待付款' : (o.order_status == 2 ? '已预约' : (o.order_status == 3 ? '已完成' : (o.order_status == 4 ? '已取消' : (o.order_status == 5 ? '申请退款' : (o.order_status == 6 ? '交易关闭' : '')))))) },
                { label: '第三方交易号', value: o.third_party_no },
                // { label: '服务状态', value: o.status },
