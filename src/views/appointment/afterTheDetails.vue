  ...
  <div v-loading="loading" class="refund-detail-container">
    <el-card v-if="refund">
      ...
      <el-row :gutter="0">
        ...
        <!-- <el-col :span="8">{{ refund.refund_way === 'ORIGINAL' ? '原路退回' : (refund.account_type_text || '无') }}</el-col> -->
        <el-col :span="8">{{ refund.reserver_phone || '无' }}</el-col>
        <el-col :span="4">收货人姓名</el-col>
        <el-col :span="8">{{ refund.reserver_user_name || '无' }}</el-col>
      </el-row>
      <el-row :gutter="0">
      ...
      <el-row :gutter="0">
        ...
        <el-col :span="20">
          ...
          <el-button v-if="refund.status==2" @click="handleRefundMoney" class="refund-btn">退款</el-button>
          ...
          <template slot="table-columns">
            <el-table-column  prop="exp_name" label="商品图片">
              <template slot-scope="scope">
                <img :src="scope.row.img_url" alt="" width="70">
              </template>
            </el-table-column>
            <el-table-column  prop="name" label="商品名称"/>
            <el-table-column  prop="total_price" label="单价"/>
            <el-table-column  prop="shop_num" label="购买数量"/>
            <el-table-column  prop="return_num" label="退货数量"/>
            <el-table-column label="小计">
              <template slot-scope="scope">
                <span>{{ scope.row.price * scope.row.shop_num }}</span>
              </template>
            </el-table-column>
          </template>
        </en-table-layout>
        ...
        <en-table-layout :tableData="refundDatas" class="pop-table">
          <template slot="table-columns">
            ...
            <el-table-column label="物流单号">
              <template slot-scope="scope">
                <span @click="looklogistics(scope.row)">{{ scope.row.shop_no }}</span>
              </template>
            </el-table-column>
    ...
    methods: {
      ...
      handleRefundMoney() {
        this.$prompt('请输入退款备注！', '提示', {
          ...
          API_Refund.refundMoney({
            order_no: this.refund.refund_no,
            refund_money: this.refundPrice,
            admin_remark: value
          }).then(response => {
      ...
      GET_RefundDetail() {
        this.loading = true
        let params = {order_no: this.$route.params.order_no}
        API_Refund.getRefundDetails(params).then(response => {
          this.loading = false
          this.refund = response
          this.paramsA = response.manager_server_vo
          if (response.refund) {
