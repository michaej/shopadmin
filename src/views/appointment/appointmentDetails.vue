  ...
  export default {
    ...
    methods: {
      GET_OrderDetail() {
        this.loading = true
        this.GET_OrderLog()
        API_appointment.getOrderDetail(this.sn).then(response => {
          this.loading = false
          this.orderDetail = response
          this.orderLog = response.logs
          ...
          [
            {
              ...
              key: 'base',
              items: [
                { label: '服务单号', value: o.order_no },
                { label: '店铺名称', value: o.shop_name },
                { label: '服务时长', value: o.server_time },
                { label: '价格', value: '￥' + f.formatPrice(o.total_price) },
                { label: '已付', value: '￥' + f.formatPrice(o.pay_money) },
                { label: '昵称', value: o.uname },
                { label: '姓名', value: o.reserve_user_name },
                { label: '手机号', value: o.reserve_phone },
                { label: '预约时间', value: f.unixToDate(o.create_time) },
                { label: '完成时间', value: f.unixToDate(o.update_time) },
                { label: '服务状态', value: o.status == 0 ? '删除' : ( o.status == 1 ? '待付款' : (o.status == 2 ? '已预约' : (o.status == 3 ? '已完成' : (o.status == 4 ? '已取消' : (o.status == 5 ? '申请退款' : (o.status == 6 ? '交易关闭' : '')))))) },
