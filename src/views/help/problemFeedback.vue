    ...
    >
      <!-- <div slot="toolbar" class="inner-toolbar">
        <div class="toolbar-btns">
          <el-button size="mini" type="primary" @click="handleOperateOrder(scope.$index, scope.row)" style="margin-left: 5px">新增问题</el-button>
      ...
      <template slot="table-columns">
        ...
        <!--下单时间-->
        <el-table-column prop="link_phone" label="联系电话"/>
        <!--完成时间-->
        <el-table-column prop="description" label="问题描述"/>
        <!--店铺名称-->
        <el-table-column prop="create_time" :formatter="MixinUnixToDate" label="反馈时间"/>
        <!--服务已付金额-->
        <!-- <el-table-column label="服务已付金额">
          <template slot-scope="scope">{{ scope.row.order_amount | unitPrice('￥') }}</template>
        </el-table-column> -->
        <!--预约人-->
        <el-table-column prop="is_replied" label="状态"/>
        <!--商品数量-->
        ...
        <el-table-column label="操作" width="150">
          <template slot-scope="scope">
            <el-button
              ...
              size="mini"
              type="primary"
              @click="handleOperateOrder(scope.$index, scope.row)">回复</el-button>
          </template>
        </el-table-column>
      </template>
      <el-pagination
  ...
  export default {
    ...
    methods: {
      ...
      GET_OrderList() {
        ...
        API_order.getCustomList(params).then(response => {
      ...
      marketStatus(row, column, cellValue) {
        ...
        if (is_auth === 0) {
          return row.market_enable === 1 ? '未回复' : '未回复'
        } else {
          return is_auth === 2 ? '已回复' : '已回复'
        }
      },
