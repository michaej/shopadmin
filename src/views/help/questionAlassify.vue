  ...
  <div>
    ...
    >
      <div slot="toolbar" class="inner-toolbar">
        <div class="toolbar-btns">
          <el-button size="mini" type="primary" @click="handleAddSpecs">新增问题分类</el-button>
        </div>
      ...
      <template slot="table-columns">
        ...
        <template slot-scope="scope">
          {{ scope.row.is_show == 0 ? '隐藏' : (scope.row.is_show == 1 ? '显示' : '')}}
          </template>
        </el-table-column>
        <!--操作-->
        <el-table-column label="操作" width="350">
          <template slot-scope="scope">
            <el-button
              size="mini"
              type="primary"
              @click="handleEditSpec(scope.$index, scope.row)">编辑</el-button>
              <el-button
              size="mini"
              type="danger"
            ...
            <el-button
              size="mini"
              type="primary"
              @click="handleEditSpecVal(scope.$index, scope.row)">新增下级</el-button>
          </template>
        </el-table-column>
      </template>
      <el-pagination
    ...
    >
      <el-form :model="specForm" :rules="specRules" ref="specForm" label-width="100px">
        <!--规格名称-->
        <el-form-item label="分类名称" prop="spec_name">
          <el-input v-model="specForm.spec_name"></el-input>
        </el-form-item>
        <!--规格备注-->
        <el-form-item label="上级分类" prop="spec_memo">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      GET_SpecsList() {
        this.loading = true
        API_spec.getClassOneList().then(response => {
          this.loading = false
      ...
      /** 删除规格 */
      DELETE_Specs(ids) {
        API_spec.deleteSpecs({id:ids}).then(response => {
