  ...
  <div>
    ...
    >
      <el-form :model="catForm" :rules="catRules" ref="catForm" label-width="120px">
        <!--分类名称-->
        <el-form-item label="店铺分类" prop="category_name">
          <el-input v-model="catForm.category_name" :maxlength="20" clearable></el-input>
        </el-form-item>
...
import * as API_goodsService from "@/api/goodsService";
  ...
  methods: {
    ...
    handleDeleteGroupBuy(index, row) {
      this.$confirm("确定要删除分类标签吗？", "提示", { type: "warning" })
        .then(() => {
          API_goodsService.deleteShopsCategorys(row.category_id).then(res => {
              this.getServeListFun();
    ...
    submitAddGroupBuyForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          const { category_id,category_name } = this.catForm;
          this.catForm.id = category_id;
          let params = {
            category_name : category_name
          }
          if (category_id) {
            API_goodsService.updateShopsCategorys(category_id, params).then(
              response => {
                if(response.code == -1){
            ...
            API_goodsService.addShopsCategorys(this.catForm).then(
              response => {
                if(response.code == -1){
                  this.$message.error(response.message);
                }else{
    ...
    //获取服务列表
    getServeListFun() {
      API_goodsService.getShopsCategorys().then(res => {
        console.log(res)
        this.catForm = res;
        this.tableData = res;
      });
