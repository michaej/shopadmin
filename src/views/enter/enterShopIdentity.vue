<template>
  <div>
    <en-table-layout :tableData="tableData" :loading="loading">
      ...
      <template slot="table-columns">
        <el-table-column prop="identity_name" label="店主身份"/>
        <el-table-column prop="identity_describe" label="介绍"/>
    ...
    >
      <el-form :model="catForm" :rules="catRules" ref="catForm" label-width="120px">
        <!--分类名称-->
        <el-form-item label="店主身份" prop="identity_name">
          <el-input v-model="catForm.identity_name" :maxlength="20" clearable></el-input>
        </el-form-item>
        <el-form-item label="介绍" prop="identity_describe">
          <el-input v-model="catForm.identity_describe" :maxlength="20" clearable></el-input>
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    handleDeleteGroupBuy(index, row) {
      this.$confirm("确定要删除分类标签吗？", "提示", { type: "warning" })
        .then(() => {
          API_goodServer.deleteShopsOwnerIdentity(row.identity_id).then(res => {
              this.getServeListFun();
    ...
    submitAddGroupBuyForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          const { identity_id,identity_name,identity_describe } = this.catForm;
          this.catForm.id = identity_id;
          let params = {
            identity_name : identity_name,
            identity_describe : identity_describe
          }
          if (identity_id) {
            API_goodServer.updateShopsOwnerIdentity(identity_id,params).then(
              response => {
                if(response.code == -1){
            ...
            API_goodServer.addShopsOwnerIdentity(this.catForm).then(
              response => {
                if(response.code == -1){
                  this.$message.error(response.message);
                }else{
    ...
    //获取店主身份
    getServeListFun() {
      API_goodServer.getShopsOwnerIdentity().then(res => {
        console.log(res)
        this.catForm = res;
        this.tableData = res;
      });
