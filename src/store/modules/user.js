...
export const mutations = {
  ...
  SET_REFRESH_TOKEN: (state, token) => {
    // const expires = new Date(jwt_decode(token).exp * 1000)
    Storage.setItem('admin_refresh_token', token, { token })
  },
  /**
   * 移除用户信息
   * @param state
  ...
  loginAction({ commit }, params) {
    return new Promise((resolve, reject) => {
      API_Login.login(params).then(response => {
        // console.log('----s-s---'+ response.token)

        // const expires = new Date(jwt_decode(response.token).exp * 1000)
        // commit('SET_USER', { ...response, expires })
        // commit('SET_ACCESS_TOKEN', response.token)
        commit('SET_REFRESH_TOKEN', response.token)
        resolve()
      }).catch(reject)
    })
  },
